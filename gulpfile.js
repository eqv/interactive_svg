 ...
 */
var SNAP_PATH = './node_modules/snapsvg/dist/';
var BUILD_PATH = './build';
var SCRIPTS_PATH = BUILD_PATH + '/scripts';
var SOURCE_PATH = './src';
var STATIC_PATH = './static';
var ENTRY_FILE = SOURCE_PATH + '/index.js';
var OUTPUT_FILE = 'template.js';
...
/**
 * Copies required Snap files from the './node_modules/snapsvg' folder into the './build/scripts' folder.
 * This way you can call 'npm update', get the lastest snap version and use it on your project with ease.
 */
function copyDeps() {

    var srcList = ['./node_modules/snapsvg/dist/snap.svg-min.js',  './node_modules/mathjax/MathJax.js', './node_modules/jquery/dist/jquery.min.js'];
    
    if (!isProduction()) {
        srcList.push('./node_modules/snapsvg/dist/snap.svg.js');
    }

    var scripts = gulp.src(srcList).pipe(gulp.dest(SCRIPTS_PATH));
    var mathjax_ext = gulp.src('./node_modules/mathjax/extensions/**/*').pipe(gulp.dest(SCRIPTS_PATH+'/extensions/'));
    var mathjax_conf = gulp.src('./node_modules/mathjax/config/**/*').pipe(gulp.dest(SCRIPTS_PATH+'/config/'));

    var mathjax_jax = gulp.src('./node_modules/mathjax/jax/**/*').pipe(gulp.dest(SCRIPTS_PATH+'/jax/'));
    return merge(scripts, mathjax_ext, mathjax_conf, mathjax_jax);
 ...
 * 
 * In order to avoid copying Snap and Static files on each build,
...
gulp.task('copyDeps', ['copyStatic'], copyDeps);
gulp.task('build', ['copyDeps'], build);
gulp.task('fastBuild', build);
gulp.task('serve', ['build'], serve);
gulp.task('watch-js', ['fastBuild'], browserSync.reload); // Rebuilds and reloads the project when executed.
gulp.task('watch-static', ['copyDeps'], browserSync.reload);

/**
 * The tasks are executed in the following order:
 * 'cleanBuild' -> 'copyStatic' -> 'copySnap' -> 'build' -> 'serve'
 * 
